...
def refine_blobs(list_blob):
    ...
    for portal_name in list_blob: #SSales_Dev #SSales_MT #HR_Assist
        if portal_name.find("user/script_v1/DotNet.ConsumerComplaints.trnCCDetails") != -1 and portal_name.find(".csv") != -1: #make change here depending on portal
            #print portal_name
...
def createMultiFileTypeLoad(rawHiveDbName,landingAreaFolder,tempStagingFolder,fileFormat,FileKey,delimiter,serde,skipHeader,res):
    ...
    for path in files:
        ...
        table_name=filenameWithoutExt[:-11] #Change before running for specific portals
        #The next command should be based on the tables names in each portal. Edit accordingly.
        just_table_name=table_name.split('.')[-1]
        table_name_wod=table_name.replace('.','_').replace('-','_').replace('[','').replace(']','')
   ...
   #hdfsFilePath="wasb://str-papl@strpapl.blob.core.windows.net/"+tempStagingFolder+"/%s" % filenameWithExt
        ...
        print " We are making "+just_table_name+ " now."
        print content
   ...
   #for j in table_name_list:
        #ower_data_type=[i.lower() for i in data_type]
        res['Table_Name']=res['Table_Name'].str.lower()
        just_table_name1=just_table_name.split('_')[-1]
        df_filtered = res[(res.Table_Name == just_table_name1.lower())]
        print "Printing Filtered dataframe " 
        print df_filtered
        flag='0'
        for i in content:
   ...
   # Create Query Formation using the header and execution in beeline shell
        ...
        try: 
            ...
            cmd="hdfs dfs -rm -r wasb://str-papl-prod@strpaplprod.blob.core.windows.net/user/antuit_user/ant_prod.db/%s" % table_name_wod
            line = os.popen(cmd,"r")
            print cmd
            dropQuery= 'DROP TABLE IF EXISTS '+rawHiveDbName+'.'+table_name_wod
            createQuery= 'Create external table '+rawHiveDbName+'.'+table_name_wod+'('+replaceDelimiterByDatatype+', ingestedtime timestamp  ) '+serdeString+' STORED AS orc '+skipHeaderString
            print "\n" +createQuery
...
if __name__ == '__main__':
  ...
  parser.read('/home/sonal/prod_conf.ini')
  ...
  #list_blob_fin1=[]
  #Calling list blob
  list_blobs('strpaplprod','Tp1Y8PsrZpVzwHMcooYacnTZKGOE7JGrFyLpPga7IAQwncUOmKyxN9zf/4g8UoaQiV58fZ7jkQnao0HbN0CpWA==','str-papl-prod')
  ...
  res,lower=read_file('/home/sonal/trnccdetails_tabledetails.csv')		
  ...
  for folder_name in list_blob_fin:
      ...
      print "Landing area folder name" + landingAreaFolder1
